...
#   ./deploy.sh --compatible       # Deploy OpenAI-compatible support files only
#   ./deploy.sh --openrouter       # Legacy alias for --compatible
...
for arg in "$@"; do
    case $arg in
        ...
        --help|-h)
            ...
            echo "  --compatible    Deploy only OpenAI-compatible support files (safe)"
            echo "  --openrouter    Legacy alias for --compatible"
            ...
            echo "  ~/setup-compatible.sh           - OpenAI-compatible setup script"
            echo "  ~/setup-openrouter.sh           - Legacy OpenRouter setup script"
...
show_status() {
    ...
    COMPATIBLE_CONFIG=$(ssh "$SERVER" "[ -f ~/.config/mini-swe-agent/.env.compatible ] && echo 'present' || echo 'not configured'")
    ...
    echo "  Compatible config:    $COMPATIBLE_CONFIG"
...
#############################################
# COMPATIBLE ONLY MODE - Safe deployment
#############################################

if [ "$COMPATIBLE_ONLY" = true ]; then
    log_step "Deploying OpenAI-compatible support files only..."
    
    # These files are NEW and don't exist on the agent's system
    # or are in locations the agent doesn't typically modify
    
    # OpenAI-compatible agent config (safe location)
    ...
    # OpenAI-compatible setup script
    scp -q "$SCRIPT_DIR/setup-compatible.sh" "$SERVER:~/setup-compatible.sh"
    ssh "$SERVER" "chmod +x ~/setup-compatible.sh"
    log_info "setup-compatible.sh (new file)"

    # Legacy OpenRouter setup script
    scp -q "$SCRIPT_DIR/setup-openrouter.sh" "$SERVER:~/setup-openrouter.sh"
    ssh "$SERVER" "chmod +x ~/setup-openrouter.sh"
    log_info "setup-openrouter.sh (legacy file)"
    ...
    HAS_COMPATIBLE=$(ssh "$SERVER" "grep -q 'run_with_compatible' ~/run_ai.sh 2>/dev/null && echo 'yes' || echo 'no'")
    
    if [ "$HAS_COMPATIBLE" = "no" ]; then
        ...
        log_info "run_ai.sh (updated with OpenAI-compatible support, backup created)"
    else
        log_info "run_ai.sh already has OpenAI-compatible support"
    fi
    
    echo ""
    log_info "OpenAI-compatible deployment complete!"
    echo ""
    echo "Next steps:"
    echo "  1. Get API key, base URL, and model name from your provider"
    echo "  2. Run: ssh $SERVER '~/setup-compatible.sh YOUR_API_KEY https://your-provider.example/v1 your-model-name'"
    echo "  3. Add to ~/ai_home/config.sh: COMPATIBLE_MODEL=\"your-model-name\""
    echo "  4. Update cron: ~/run_ai.sh compatible"
    exit 0
fi
...
if [ "$FORCE_MODE" = true ]; then
    ...
    if ssh "$SERVER" "[ ! -f ~/run_ai.sh ]" 2>/dev/null; then
        ...
        # Check if it needs OpenAI-compatible update
        HAS_COMPATIBLE=$(ssh "$SERVER" "grep -q 'run_with_compatible' ~/run_ai.sh 2>/dev/null && echo 'yes' || echo 'no'")
        if [ "$HAS_COMPATIBLE" = "no" ]; then
            log_warn "run_ai.sh exists but lacks OpenAI-compatible support. Use --compatible to update safely."
        else
            log_skip "run_ai.sh"
        fi
    fi
